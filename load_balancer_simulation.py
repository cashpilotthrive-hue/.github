...
class Server:
    ...
    def reserve_connection(self, request_id):
        ...
        # BOLT: Logging and calculations moved outside the lock to reduce contention.
        logging.info(f"Request {request_id} assigned to {self.name}. Active connections: {connections}/{self.capacity}")
        if connections >= self.high_load_threshold:
            logging.warning(f"ALERT: Server {self.name} is approaching high load: {connections / self.capacity:.2%}")
        return True
...
class LoadBalancer:
    ...
    def select_server(self):
        ...
        for server in self.servers:
            # BOLT: Direct attribute access and early exit for speed.
            current_load = server.active_connections
            if current_load < min_connections:
