...
class Server:
    def __init__(self, name, capacity=10):
        ...
        self.heap_index = -1
        # Pre-calculate threshold to avoid floating-point division in the hot path
    ...
    def handle_request(self, request_id, processing_time, current_connections):
        ...
        logging.info(
            ...
            f"Active connections: {current_connections}/{self.capacity} "
            f"(CPU load: {cpu_load:.2%})"
        )

        # Use pre-calculated threshold and captured count for speed and consistency
...
class LoadBalancer:
    ...
    def route_request(self, request_id, processing_time):
        ...
        with self.lock:
            ...
            if server:
                if server.active_connections < server.capacity:
                    ...
                    # Mark for rejection, construction of message happens outside the lock
                    rejection_reason = "capacity"
            else:
                rejection_reason = "no_servers"

        if rejection_reason:
            if rejection_reason == "capacity":
                logging.error(
        ...
        if assigned_server:
            try:
                # Use captured current_load for consistency and to avoid extra attribute access
                assigned_server.handle_request(
                    request_id, processing_time, current_load
                )
            finally:
