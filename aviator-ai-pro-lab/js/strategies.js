...
class StrategyEngine {
  ...
  backtest(strategyKey, crashPoints, bankroll = 1000, includeResults = true) {
    ...
    for (let i = 0; i < crashPoints.length; i++) {
      ...
      // BOLT OPTIMIZATION: Skip result object creation during iterative optimization
      if (includeResults) {
        results.push({
          round: i + 1,
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Consolidate pattern analysis into a single O(N) pass
    let sum = 0, sumSq = 0, recentSum = 0, lowCrashCount = 0;
    const count = crashes.length;
    const recentStartIndex = Math.max(0, count - 5);
    ...
    const recentAvg = recentSum / (count - recentStartIndex);
