...
class StrategyEngine {
  ...
  backtest(strategyKey, crashPoints, bankroll = 1000, options = {}) {
    ...
    for (let i = 0; i < crashPoints.length; i++) {
      ...
      // BOLT OPTIMIZATION: Skip population of results array when includeResults is false
      if (includeResults) {
        results.push({
          round: i + 1,
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Calculate average, variance, recent average, and low crash count
    // in a single pass to completely avoid redundant array slice/filter/reduces.
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        // BOLT OPTIMIZATION: Disable results array population during iterations
    ...
    // Restore original parameters to the engine's internal state
    this.strategies[strategyKey] = { ...strategy, params: originalParams };

    // BOLT OPTIMIZATION: Perform a final high-fidelity backtest with results included for UI visualization
    if (bestParams) {
