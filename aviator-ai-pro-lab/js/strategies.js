...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Calculate mean, variance, recent mean, and low crash ratio
    // using direct loops to avoid array allocations (slice, filter) and multiple passes over memory.
    let sum = 0;
    for (let i = 0; i < len; i++) {
      sum += crashes[i];
    }
