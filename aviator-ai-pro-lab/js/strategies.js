...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Consolidate sum, lowCrash count, and recentAvg calculations
    // into a single O(N) loop to eliminate redundant array allocations (slice, filter) and passes.
    let sum = 0;
    let lowCount = 0;
    let recentSum = 0;
    ...
    // BOLT OPTIMIZATION: Use Math.round instead of toFixed for ~5x faster rounding without string allocations
    return {
