...
class StrategyEngine {
  ...
  backtest(strategyKey, crashPoints, bankroll = 1000, options = {}) {
    ...
    for (let i = 0; i < crashPoints.length; i++) {
      ...
      // BOLT OPTIMIZATION: Skip large array allocation for high-performance optimization iterations.
      if (includeResults) {
        results.push({
          round: i + 1,
    ...
    const totalRounds = roundsPlayed;
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Calculate all AI metrics in a single O(N) pass to avoid
    // multiple iterations and array slicing/filtering overhead.
    let sum = 0;
    let sumSq = 0;
    let recentSum = 0;
    let lowCrashCount = 0;
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    // to drastically reduce memory overhead and array manipulation time.
    ...
    // BOLT OPTIMIZATION: Run one final high-fidelity backtest for the best found parameters
    // to provide a complete result object (with results array) for UI visualization.
    if (bestParams) {
      ...
      // Keep bestParams applied if we want the engine to use them, or restore?
      // UI usually wants the engine to stay in its previous state or use the best.
      // Based on original code, it restores.
      this.strategies[strategyKey].params = originalParams;
    }
