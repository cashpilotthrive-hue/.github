...
class StrategyEngine {
  ...
  backtest(strategyKey, crashPoints, bankroll = 1000, options = {}) {
    ...
    for (let i = 0; i < crashPoints.length; i++) {
      ...
      // BOLT OPTIMIZATION: Use _round helper for faster rounding and skip array when not needed
      if (includeResults) {
        results.push({
          round: i + 1,
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Single pass for mean, variance, momentum, and low crash ratio
    let sum = 0;
    let sumSq = 0;
    let recentSum = 0;
    let lowCrashCount = 0;
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        // BOLT OPTIMIZATION: Skip result array generation during optimization runs
    ...
    // Restore original and then perform one final full backtest with best params
