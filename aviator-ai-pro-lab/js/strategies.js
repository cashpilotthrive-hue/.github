...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Consolidate multiple array passes (avg, recentAvg, lowCrashRatio)
    // into a single O(N) loop and replace slow toFixed() with mathematical rounding.
    let sum = 0;
    let lowCrashCount = 0;
    let recentSum = 0;
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        // BOLT OPTIMIZATION: Skip building results array during optimization for speed
