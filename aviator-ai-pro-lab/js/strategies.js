...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Consolidate sum, recentSum (last 5 crashes), and lowCrashCount
    // into a single O(N) pass to avoid temporary array allocations (slice/filter/reduce).
    let sum = 0;
    let recentSum = 0;
    let lowCrashCount = 0;
    ...
    const avg = sum / len;
    const recentAvg = recentSum / (len < 5 ? len : 5);
    ...
    const volatility = Math.sqrt(sumSq / len);
    ...
    // BOLT OPTIMIZATION: Use Math.round instead of parseFloat(toFixed()) for 20x faster formatting
    return {
