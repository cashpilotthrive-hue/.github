...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Consolidate average, variance, recentAvg, and lowCrashRatio
    // into a single O(N) loop to avoid multiple reduce/slice/filter allocations.
    const len = crashes.length;
    let sum = 0;
    let sumSq = 0;
    let recentSum = 0;
    let lowCrashCount = 0;
    const recentStartIdx = len - 5;
    ...
    const avg = sum / len;
    // Single-pass variance: (sumSq / len) - (avg * avg) with Math.max for stability.
    const variance = Math.max(0, (sumSq / len) - (avg * avg));
    const volatility = Math.sqrt(variance);

    const recentCount = len - Math.max(0, recentStartIdx);
    ...
    // BOLT OPTIMIZATION: Use deterministic math rounding helper for a ~3x speedup over parseFloat/toFixed
    return {
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    // BOLT OPTIMIZATION: Capture original parameters as a stable baseline for randomization,
    // avoiding random walk state degradation during iterations.
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        // BOLT OPTIMIZATION: Run optimized fast backtests during the iterations loop
    ...
    // Restore original parameters to the strategy engine
    this.strategies[strategyKey].params = originalParams;

    // BOLT OPTIMIZATION: Execute a single final high-fidelity backtest using bestParams
    // to populate the returned bestResult with results array needed for the frontend.
