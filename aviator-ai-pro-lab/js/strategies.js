...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Calculate avg, recentAvg, and lowCrashRatio in a single O(N) loop
    // to eliminate slice(), filter(), reduce() array allocations and closures.
    let sum = 0;
    let recentSum = 0;
    let lowCrashCount = 0;
    ...
    const avg = sum / len;
    const recentAvg = recentSum / (len - recentStartIndex);
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        // BOLT OPTIMIZATION: Pass includeResults: false during optimization iterations to eliminate array allocations.
    ...
    // BOLT OPTIMIZATION: Run a final high-fidelity backtest with includeResults: true for the optimal parameters
    // to ensure full UI visualization capability while keeping iteration runs ultra-fast.
    if (bestParams) {
