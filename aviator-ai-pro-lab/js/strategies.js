...
class StrategyEngine {
  ...
  backtest(strategyKey, crashPoints, bankroll = 1000, options = { includeResults: true }) {
    ...
    const results = options.includeResults ? [] : null;
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Single-pass calculation of mean, variance and low crash ratio
    let sum = 0;
    let sumSq = 0;
    let lowCrashCount = 0;
    for (let i = 0; i < n; i++) {
    ...
    const confidence = Math.min(0.95, 0.3 + (n / state.adaptiveWindow) * 0.5 - volatility * 0.05);
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    for (let i = 0; i < iterations; i++) {
      // BOLT OPTIMIZATION: Use originalParams for randomization base to prevent drift
      const params = this._randomizeParams(strategyKey, originalParams);
      this.strategies[strategyKey].params = params;

      try {
        // BOLT OPTIMIZATION: Skip results array generation during optimization iterations (saves ~80% time/memory)
    ...
    // Restore original and then perform one final high-fidelity backtest with best params
    this.strategies[strategyKey].params = bestParams || originalParams;
    const finalResult = this.backtest(strategyKey, crashPoints, bankroll, { includeResults: true });
    if (bestResult) finalResult.score = bestResult.score;

    this.strategies[strategyKey].params = originalParams;

    return {
      bestParams: bestParams || originalParams,
