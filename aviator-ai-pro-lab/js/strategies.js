...
class StrategyEngine {
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    // BOLT OPTIMIZATION: Capture original params to avoid randomization drift
    const originalParams = { ...strategy.params };

    for (let i = 0; i < iterations; i++) {
      // Use includeResults: false during optimization to avoid ~100k object allocations
    ...
    // Reset strategy to original params
    this.strategies[strategyKey] = { ...strategy, params: originalParams };

    // Perform one final full backtest with the best parameters to get the results for the UI
    if (bestParams) {
