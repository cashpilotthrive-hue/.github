...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Consolidate multiple array passes (avg, variance, momentum, ratio)
    ...
    for (let i = 0; i < len; i++) {
      ...
      if (i >= len - recentLimit) recentSum += c;
    }
    ...
    // BOLT OPTIMIZATION: Use mathematical rounding instead of slow toFixed()
    return {
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        // BOLT OPTIMIZATION: Use includeResults: false during iterative optimization
        // to skip expensive object allocations and result array creation.
    ...
    // Restore original parameters and perform one final full backtest with results
    // for the best found parameters so the UI can display the simulation path.
    this.strategies[strategyKey] = { ...strategy, params: bestParams };
    const finalBestResult = bestParams ? this.backtest(strategyKey, crashPoints, bankroll, true) : null;
    if (finalBestResult) finalBestResult.score = bestResult.score;
