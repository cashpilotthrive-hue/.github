...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // and lowCrashRatio into a single O(N) pass, avoiding redundant array allocations (slice, filter)
    // and multiple reduce iterations.
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        // Fast backtest without results array creation
