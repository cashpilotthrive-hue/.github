...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Consolidate Calculations of average, variance, recentAvg,
    // and lowCrashRatio into a single O(N) pass, avoiding redundant slice, filter and reduce passes.
    let sum = 0;
    let sumSq = 0;
    let lowCrashCount = 0;
    let recentSum = 0;
    ...
    const avg = sum / count;
    // Single-pass variance using: E[X^2] - E[X]^2
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      const tempStrategy = { ...strategy, params };
      this.strategies[strategyKey] = tempStrategy;

      try {
        // BOLT OPTIMIZATION: Skip results array creation during iterations to save CPU and memory
    ...
    // Restore original parameters to the strategy engine instance
    this.strategies[strategyKey] = { ...strategy, params: originalParams };

    if (bestParams) {
      // Run high-fidelity backtest to return full visualization details
