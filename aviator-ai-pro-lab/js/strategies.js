...
class StrategyEngine {
   ...
   * @param {boolean} includeResults - Whether to build the detailed results array (faster if false)
   */
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Consolidate multiple passes (avg, variance, momentum, lowCrashRatio)
    // into a single O(N) loop for significantly better performance.
    let sum = 0;
    let sumSq = 0;
    let lowCrashCount = 0;
    let recentSum = 0;
    ...
    for (let i = 0; i < len; i++) {
      ...
      if (i >= len - recentCount) recentSum += c;
    }

    const avg = sum / len;
    // Use variance formula: E[X^2] - (E[X])^2
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        ...
        // to avoid expensive object allocations and array operations.
    ...
    // Perform one final backtest with best parameters to get full results for UI display
    let finalBestResult = null;
    if (bestParams) {
      const originalStrategyParams = this.strategies[strategyKey].params;
      this.strategies[strategyKey].params = bestParams;
      finalBestResult = this.backtest(strategyKey, crashPoints, bankroll, true);
      finalBestResult.score = bestResult.score; // Carry over the score
      this.strategies[strategyKey].params = originalStrategyParams;
    }
