...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Calculate sum, recentSum, and lowCrashRatio in a single O(N) pass,
    // avoiding redundant array allocations (.slice, .filter) and multiple .reduce passes.
    let sum = 0;
    let lowCount = 0;
    let recentSum = 0;
    ...
    // BOLT OPTIMIZATION: Fast mathematical rounding instead of parseFloat(x.toFixed(n))
    return {
