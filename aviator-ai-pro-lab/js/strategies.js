...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Calculate sum, recentSum (last 5), and lowCrashCount in a single pass
    // avoiding array allocations from slice(-5) and filter(c => c < 1.5).
    let sum = 0;
    let recentSum = 0;
    let lowCrashCount = 0;
    ...
    // BOLT OPTIMIZATION: Use Math.round instead of parseFloat(toFixed(...)) to avoid string allocations
    return {
