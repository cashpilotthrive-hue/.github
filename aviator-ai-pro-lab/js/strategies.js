...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Calculate average, variance, low crash count,
    // and recent average (last 5) in a single unified O(N) pass.
    let sum = 0;
    let sumSq = 0;
    let lowCrashCount = 0;
    let recentSum = 0;
    ...
    return {
      ...
      // BOLT OPTIMIZATION: Use custom math _round instead of expensive toFixed conversions.
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    let bestResult = null;
    let bestParams = null;
    // Capture the base parameters to prevent a cumulative random walk
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        // BOLT OPTIMIZATION: Run fast backtest without constructing heavy results array during search iterations
    ...
    // Run a final high-fidelity backtest to return fully populated results to the UI
