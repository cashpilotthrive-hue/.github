...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Single O(N) pass for avg, variance, recentAvg, and lowCrashRatio
    // to eliminate redundant array allocations (slice/filter) and multiple reduce calls.
    let sum = 0;
    let sumSq = 0;
    let sumRecent = 0;
    let lowCrashCount = 0;
    ...
    // BOLT OPTIMIZATION: Fast math-based rounding instead of string toFixed conversion
    return {
  ...
  _randomizeParams(key, baseParams) {
    ...
    // BOLT OPTIMIZATION: Replace parseFloat(rand(...).toFixed(...)) with fast Math.round math rounding
