...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Consolidate standard deviation, variance, recent average, and low crash ratio into a single O(N) pass.
    let sum = 0;
    let sumSq = 0;
    let lowCrashCount = 0;
    let recentSum = 0;
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    // Run final high-fidelity backtest with includeResults: true to fully populate bestResult
    if (bestParams) {
      ...
      bestResult.score = score; // Restore optimization score
