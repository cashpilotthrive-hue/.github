...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // Single-pass optimization: consolidate average, variance, recentAvg, and lowCrashRatio
    let sum = 0;
    let sumSq = 0;
    let recentSum = 0;
    let lowCrashCount = 0;
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    // Capture original params before modifying them during iterations to avoid shared object mutation.
    const originalParams = { ...strategy.params };

    for (let i = 0; i < iterations; i++) {
      // Randomize params starting from original base params
      ...
      try {
        // Fast backtest skipping results array creation
    ...
    // Run a final high-fidelity backtest with includeResults: true after identifying optimal parameters
    if (bestParams) {
