...
class StrategyEngine {
  ...
  backtest(strategyKey, crashPoints, bankroll = 1000, options = {}) {
    ...
    for (let i = 0; i < crashPoints.length; i++) {
      ...
      // BOLT OPTIMIZATION: Skip results array population for faster AI iterations
      if (includeResults) {
        results.push({
          round: i + 1,
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Calculate all AI analysis metrics in a single pass O(N) instead of multiple O(N) passes
    let sum = 0;
    let sumSq = 0;
    let recentSum = 0;
    let lowCrashCount = 0;
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        // BOLT OPTIMIZATION: Use fast backtest (no results array) for optimization iterations
    ...
    // BOLT OPTIMIZATION: Run one final full backtest with the best parameters to return complete results
    if (bestParams) {
      ...
      this.strategies[strategyKey].params = strategy.params; // Restore original
    }
