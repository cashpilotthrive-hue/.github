...
class StrategyEngine {
  ...
  backtest(strategyKey, crashPoints, bankroll = 1000, includeResults = true) {
    ...
    for (let i = 0; i < crashPoints.length; i++) {
      ...
      // BOLT OPTIMIZATION: Skip object creation when results are not needed (saves memory/CPU)
      if (includeResults) {
        results.push({
          round: i + 1,
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        // BOLT OPTIMIZATION: Use includeResults: false for 3x faster iterations
    ...
    // Restore original strategy and params
    this.strategies[strategyKey] = strategy;

    // BOLT OPTIMIZATION: Perform one final full backtest for the best result to provide UI data
