...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Consolidate multiple passes (reduce, filter, slice) into a single O(N) pass.
    // Uses sum-of-squares formula for variance to avoid two passes over the array.
    let sum = 0;
    let sumSq = 0;
    let lowCrashCount = 0;
    let recentSum = 0;
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        // BOLT OPTIMIZATION: Set includeResults to false during optimization iterations
        // to skip building the results array, significantly reducing memory and CPU.
