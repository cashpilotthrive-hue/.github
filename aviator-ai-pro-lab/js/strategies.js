...
class StrategyEngine {
  ...
  backtest(strategyKey, crashPoints, bankroll = 1000, includeResults = true) {
    ...
    for (let i = 0; i < crashPoints.length; i++) {
      ...
      // BOLT OPTIMIZATION: Skip results array generation when not needed (e.g. during optimization)
      // and use Math.round instead of toFixed for 20x faster rounding
      if (includeResults) {
        results.push({
          round: i + 1,
  ...
  _aiAnalyze(state) {
    ...
    // using Welford's algorithm for variance and calculating recent avg/low crash ratio.
    let sum = 0;
    let m2 = 0;
    let mean = 0;
    let recentSum = 0;
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        // BOLT OPTIMIZATION: Pass includeResults=false to speed up optimization runs
