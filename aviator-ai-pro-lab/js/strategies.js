...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Consolidate sum, low crash counting, and recent average in a single O(N) loop
    // to eliminate redundant array allocations (`slice`, `filter`) and multiple `reduce` iterations (~4x speedup).
    let sum = 0;
    let lowCrashCount = 0;
    let recentSum = 0;
    ...
    // BOLT OPTIMIZATION: Use Math.round instead of parseFloat(n.toFixed(...)) string conversion
    return {
