...
class StrategyEngine {
  ...
  backtest(strategyKey, crashPoints, bankroll = 1000, options = {}) {
    ...
    for (let i = 0; i < crashPoints.length; i++) {
      ...
      if (includeResults) {
        // BOLT OPTIMIZATION: Use _round instead of Math.round/100 or toFixed
        results.push({
          round: i + 1,
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        // BOLT OPTIMIZATION: Use fast path (no results array) during iterations
    ...
    // Restore original or set to best for final high-fidelity run
    this.strategies[strategyKey].params = bestParams || originalParams;

    // Final high-fidelity run to get full results for UI
    const bestResult = bestParams
