...
class StrategyEngine {
   ...
   * BOLT OPTIMIZATION: includeResults flag to skip detailed round history during optimization
   */
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Calculate all metrics in a single O(N) pass
    // to avoid multiple reduce/filter/slice/pow calls.
    let sum = 0;
    let sumSq = 0;
    let recentSum = 0;
    let lowCrashes = 0;
    ...
    return {
      ...
      // BOLT OPTIMIZATION: Math.round is faster than toFixed().parseFloat()
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        // BOLT OPTIMIZATION: Pass includeResults = false to skip round object creation
