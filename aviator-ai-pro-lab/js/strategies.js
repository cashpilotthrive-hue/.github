...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Consolidate calculations of avg, variance, recentAvg, and lowCrashRatio
    // into a single O(N) pass to avoid redundant array allocations (slice, filter) and multiple reduce passes.
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        // BOLT OPTIMIZATION: Pass includeResults: false to avoid allocating/populating the results array
        // on every optimization iteration, saving substantial memory and time.
    ...
    // Run a final high-fidelity backtest using the best parameters to fully populate bestResult with its results array
    if (bestParams) {
