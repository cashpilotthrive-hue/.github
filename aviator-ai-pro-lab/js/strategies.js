...
class StrategyEngine {
   ...
   * BOLT OPTIMIZATION: Added includeResults parameter to skip building large arrays
   * during intensive optimization passes.
   */
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Consolidate multiple array passes (reduce, slice, filter)
    // into a single O(N) pass and use one-pass variance calculation.
    let sum = 0;
    let sumSq = 0;
    let lowCrashCount = 0;
    let recentSum = 0;
    ...
    const avg = sum / len;
    const volatility = Math.sqrt(Math.max(0, (sumSq / len) - (avg * avg)));
    ...
    // BOLT OPTIMIZATION: Use math rounding instead of toFixed() for ~20x faster processing.
    return {
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        // BOLT OPTIMIZATION: Use the fast path backtest (skipping results array) during optimization
