...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Calculate average, recent average, and low crash ratio in a single O(N) pass
    // without array slicing (`.slice`), filtering (`.filter`), or multiple `.reduce` allocations.
    let sum = 0;
    let recentSum = 0;
    let lowCrashCount = 0;
    ...
    // BOLT OPTIMIZATION: Use direct math rounding to avoid string allocations via parseFloat(toFixed()).
    return {
