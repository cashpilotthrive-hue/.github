...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Calculate avg, variance, recentAvg, and lowCrashRatio in a single O(N) pass
    let sum = 0;
    let sumSq = 0;
    let recentSum = 0;
    let lowCrashCount = 0;
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    for (let i = 0; i < iterations; i++) {
      // BOLT OPTIMIZATION: Use originalParams as base to avoid random walk degradation
      const params = this._randomizeParams(strategyKey, originalParams);
      this.strategies[strategyKey] = { ...strategy, params };

      try {
        // BOLT OPTIMIZATION: Pass includeResults: false during search to avoid massive array allocations
    ...
    if (bestParams) {
      // Run final high-fidelity backtest with best parameters to get full results for UI
