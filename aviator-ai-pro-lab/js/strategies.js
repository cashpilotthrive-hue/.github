...
class StrategyEngine {
  ...
  backtest(strategyKey, crashPoints, bankroll = 1000, includeResults = true) {
    ...
    for (let i = 0; i < crashPoints.length; i++) {
      ...
      // BOLT OPTIMIZATION: Skip creating results array during iterative optimization (O(1) memory)
      if (includeResults) {
        results.push({
          round: i + 1,
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Single-pass calculation of all stats (O(N))
    let sum = 0;
    let sumSq = 0;
    let recentSum = 0;
    let lowCrashCount = 0;
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    // Preserve original params to avoid state corruption during iteration
    const originalParams = { ...strategy.params };
    let bestResult = null;
    let bestParams = null;

    for (let i = 0; i < iterations; i++) {
      // Use originalParams as base for randomization to ensure independent sampling
      const params = this._randomizeParams(strategyKey, originalParams);
      strategy.params = params;

      try {
        // BOLT OPTIMIZATION: Use includeResults: false for iterative optimization (faster/less memory)
    ...
    // Final backtest with best parameters to return full results for UI display
    ...
    if (bestParams) {
      ...
      strategy.params = originalParams; // Restore again
    }
