...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Calculate avg, variance, momentum and lowCrashRatio in a single pass O(N)
    let sum = 0;
    let sumSq = 0;
    let recentSum = 0;
    let lowCrashCount = 0;
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    // BOLT OPTIMIZATION: Capture original params via shallow copy before loop to prevent state mutation
    const originalParams = { ...strategy.params };
    let bestScore = -Infinity;
    let bestParams = null;

    for (let i = 0; i < iterations; i++) {
      // Use originalParams as base for randomization to avoid "random walk" drift
    ...
    // Final high-fidelity backtest with best parameters to get full results for UI
