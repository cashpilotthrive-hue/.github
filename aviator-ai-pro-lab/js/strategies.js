...
class StrategyEngine {
  ...
  backtest(strategyKey, crashPoints, bankroll = 1000, options = { includeResults: true }) {
    ...
    for (let i = 0; i < crashPoints.length; i++) {
      ...
      // BOLT OPTIMIZATION: Skip results array creation during iterative optimization
      if (includeResults) {
        results.push({
          round: i + 1,
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Consolidate multiple passes into a single O(N) loop
    let sum = 0;
    let sumSq = 0;
    let recentSum = 0;
    let lowCrashes = 0;
    ...
    const lowCrashRatio = lowCrashes / len;
    ...
    return {
      ...
      // BOLT OPTIMIZATION: Replace slow toFixed with math rounding
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      // Temporarily set params for the backtest
      this.strategies[strategyKey].params = params;

      try {
        // BOLT OPTIMIZATION: Pass includeResults: false to avoid massive array allocations
    ...
    // Run one final backtest with best params and includeResults: true for UI display
