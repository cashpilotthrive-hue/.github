...
class StrategyEngine {
  ...
  backtest(strategyKey, crashPoints, bankroll = 1000, options = {}) {
    ...
    for (let i = 0; i < crashPoints.length; i++) {
      ...
      // BOLT OPTIMIZATION: Skip results array creation and allocations if not requested
      if (includeResults) {
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Consolidate stats calculations into a single O(N) pass to avoid multiple array traversals and slice allocations.
    let sum = 0;
    let sumSq = 0;
    let recentSum = 0;
    let lowCrashCount = 0;
    ...
    const recentStartIndex = count - recentCount;
    ...
    const avg = sum / count;
    // (sumSq / count) - (avg * avg) with Math.max(0, variance) for stability
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    let bestResult = null;
    let bestParams = null;
    // BOLT OPTIMIZATION: Capture originalParams before the loop to prevent dependent random walks due to mutated params
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        // BOLT OPTIMIZATION: Skip results array creation for iteration runs
    ...
    // Restore strategy back to original parameters temporarily
    this.strategies[strategyKey] = { ...strategy, params: originalParams };

    // BOLT OPTIMIZATION: Run a final high-fidelity backtest with best parameters to populate the results array for UI rendering
    if (bestParams) {
      const bestStrategy = { ...this.strategies[strategyKey], params: bestParams };
      this.strategies[strategyKey] = bestStrategy;
    ...
    // Ensure the engine's original state is fully restored
    this.strategies[strategyKey] = { ...strategy, params: originalParams };
