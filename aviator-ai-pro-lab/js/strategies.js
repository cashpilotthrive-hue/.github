...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Single pass calculation of metrics to avoid redundant array iterations
    let sum = 0;
    let sumSq = 0;
    let recentSum = 0;
    let lowCrashCount = 0;
    ...
    // BOLT OPTIMIZATION: Faster mathematical rounding instead of slow toFixed()
    return {
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        // BOLT OPTIMIZATION: Skip detailed results in optimization loop to speed up iterations
