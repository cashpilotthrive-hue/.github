...
class StrategyEngine {
  ...
  /**
   * Helper function for faster rounding replacing parseFloat(n.toFixed(decimals))
   */
  ...
  backtest(strategyKey, crashPoints, bankroll = 1000, options = {}) {
    ...
    for (let i = 0; i < crashPoints.length; i++) {
      ...
      if (includeResults) {
        // BOLT OPTIMIZATION: Use Math.round/this._round instead of toFixed for 20x faster rounding
        results.push({
          round: totalRounds,
    ...
    return {
      ...
      roi: this._round((totalProfit / bankroll * 100) * 100) / 100,
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Consolidate average, variance, recentAvg (last 5), and low crash ratio calculations
    // into a single O(N) loop to eliminate multiple array passes and redundant slice/filter allocations.
    let sum = 0;
    let sumSq = 0;
    let lowCrashCount = 0;
    let recentSum = 0;
    const recentLimit = Math.min(len, 5);
    const recentThreshold = len - recentLimit;
    ...
    const avg = sum / len;
    // Single-pass variance formula: E[X^2] - (E[X])^2
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    // BOLT OPTIMIZATION: Shallow copy originalParams before entering the loop
    // to ensure we restore the original parameters exactly when finished.
    ...
    for (let i = 0; i < iterations; i++) {
      // Use originalParams as base to avoid randomized random-walk parameter drift
      const params = this._randomizeParams(strategyKey, originalParams);
      strategy.params = params;

      try {
        // BOLT OPTIMIZATION: Run iterations with includeResults: false to prevent large array allocations
    ...
    // Restore the original parameters to the engine's active configuration
    strategy.params = originalParams;

    // BOLT OPTIMIZATION: Run a final high-fidelity backtest with includeResults: true
    // for the identified best parameters, ensuring we return a fully-populated bestResult object for UI visual usage
    if (bestParams) {
