...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Single-pass O(N) loop for sum, lowCrashRatio, and recentAvg calculations
    let sum = 0;
    let lowCount = 0;
    let recentSum = 0;
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        // BOLT OPTIMIZATION: Pass includeResults: false during optimization loop to skip array allocation overhead
    ...
    // BOLT OPTIMIZATION: Perform a final high-fidelity backtest with full results for UI rendering
    this.strategies[strategyKey] = { ...strategy, params: bestParams || strategy.params };
