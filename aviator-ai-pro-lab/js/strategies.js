...
class StrategyEngine {
   ...
   * @param {Object} options - Optimization options { includeResults: boolean }
   */
  backtest(strategyKey, crashPoints, bankroll = 1000, options = {}) {
    ...
    for (let i = 0; i < crashPoints.length; i++) {
      ...
      // BOLT OPTIMIZATION: Skip result array population during optimization to save memory and CPU
      if (includeResults) {
        results.push({
          round: i + 1,
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Calculate all metrics in a single O(N) pass instead of multiple array iterations
    let sum = 0;
    let sumSq = 0;
    let recentSum = 0;
    let lowCrashCount = 0;
    ...
    const avg = sum / count;
    // Single-pass variance: E[X^2] - (E[X])^2
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    // BOLT OPTIMIZATION: Run optimized backtests without results array creation in the loop
    ...
    // Perform one final high-fidelity backtest with best params to get full results
    if (bestParams) {
