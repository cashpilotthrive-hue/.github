...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Calculate average, variance, momentum, and ratio in a single O(N) pass.
    // Uses the sum-of-squares formula for variance to avoid multiple array iterations.
    let sum = 0;
    let sumSq = 0;
    let lowCount = 0;
    let recentSum = 0;
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        // BOLT OPTIMIZATION: Skip building the results array during optimization
        // to reduce memory overhead and CPU time (O(1) vs O(N) per round).
