...
class StrategyEngine {
  ...
  backtest(strategyKey, crashPoints, bankroll = 1000, includeResults = true) {
    ...
    for (let i = 0; i < crashPoints.length; i++) {
      ...
      // BOLT OPTIMIZATION: Skip building the results array during optimization to save memory and CPU
      if (includeResults) {
        results.push({
          round: i + 1,
      ...
      this._updateState(strategyKey, state, won, crashPoint, results || EMPTY_ARRAY);
    }
  ...
  _aiAnalyze(state) {
    ...
    // into a single O(N) pass to minimize array traversals.
