...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Consolidate average, variance, recentAvg, and lowCrashRatio calculations
    // into a single O(N) pass, completely avoiding slice(), filter(), and multiple reduces.
    let sum = 0;
    let sumSq = 0;
    let recentSum = 0;
    let lowCrashCount = 0;
    const recentStartIdx = count - 5;
    ...
    const recentCount = count < 5 ? count : 5;
    ...
    // BOLT OPTIMIZATION: Use the mathematical rounding helper _round instead of parsing toFixed.
    return {
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    for (let i = 0; i < iterations; i++) {
      // BOLT OPTIMIZATION: Use originalParams as the base for randomization to avoid state mutation degrading into random walk.
      ...
      try {
        // BOLT OPTIMIZATION: Run backtest with includeResults: false to avoid array allocation during optimization.
    ...
    // Restore original parameters to avoid persisting randomized state.
    ...
    if (bestParams) {
      ...
      // using the optimal parameters so that we return a fully populated results object for the UI/charts.
      ...
      finalResult.score = bestResult.score; // Restore score
