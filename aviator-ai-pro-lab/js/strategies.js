...
class StrategyEngine {
  ...
  backtest(strategyKey, crashPoints, bankroll = 1000, includeResults = true) {
    ...
    for (let i = 0; i < crashPoints.length; i++) {
      ...
      // BOLT OPTIMIZATION: Skip results array creation for optimization runs to save memory and time
      if (includeResults) {
        results.push({
          round: i + 1,
    ...
    const totalRounds = includeResults ? results.length : wins + losses;
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Consolidate multiple passes into a single O(N) pass using Welford's algorithm
    let sum = 0, mean = 0, m2 = 0, lowCrashCount = 0, recentSum = 0;
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        // BOLT OPTIMIZATION: Skip creating results array during optimization for 75% speedup
