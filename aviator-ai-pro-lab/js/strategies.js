...
class StrategyEngine {
  ...
  backtest(strategyKey, crashPoints, bankroll = 1000, includeResults = true) {
    ...
    for (let i = 0; i < crashPoints.length; i++) {
      ...
      // BOLT OPTIMIZATION: Optionally skip building the results array to speed up optimization iterations
      if (includeResults) {
        results.push({
          round: i + 1,
  ...
  _aiAnalyze(state) {
    ...
    // to calculate average, volatility, momentum, and low crash ratio efficiently.
    let sum = 0;
    let sumSq = 0;
    let recentSum = 0;
    let lowCrashes = 0;
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        // BOLT OPTIMIZATION: Disable results array generation during optimization runs
        // to significantly reduce memory allocation overhead.
