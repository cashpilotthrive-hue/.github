...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Consolidate calculations of avg, variance, recentAvg,
    // and lowCrashRatio into a single O(N) pass to avoid redundant slice/filter allocations
    // and multiple reduce loops.
    let sum = 0;
    let sumSq = 0;
    let sumRecent = 0;
    let lowCrashCount = 0;
    const recentLimit = Math.min(count, 5);
    const startRecentIdx = count - recentLimit;
    ...
    const avg = sum / count;
    // single-pass variance: (sumSq / count) - (avg * avg)
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    for (let i = 0; i < iterations; i++) {
      // BOLT OPTIMIZATION: Capture originalParams via shallow copy and use it as base
      // to ensure _randomizeParams uses a consistent base, preventing randomization from degrading.
      const params = this._randomizeParams(strategyKey, originalParams);
      strategy.params = params;

      try {
        // BOLT OPTIMIZATION: Skip results array creation inside loop iterations to avoid large memory overhead.
    ...
    // Restore original parameters to the engine
    strategy.params = originalParams;

    let bestResult = null;
    if (bestParams) {
      // BOLT OPTIMIZATION: Run a final high-fidelity backtest with includeResults: true using bestParams
      // to ensure that the returned bestResult object is fully populated for UI visualization.
