...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // Single-pass statistical calculation for maximum performance
    let sum = 0;
    let sumSq = 0;
    let lowCrashCount = 0;
    let recentSum = 0;
    ...
    const avg = sum / len;
    // (sumSq / len) - (avg * avg) is the variance formula.
    // Math.max is used for numerical stability against tiny negative float representation.
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    // Capture original params via shallow copy to maintain consistent base
    ...
    if (bestParams) {
      // Run final high-fidelity backtest with best parameters
      ...
      // Restore original parameters to the strategy again
      this.strategies[strategyKey] = { ...strategy, params: originalParams };
    }
