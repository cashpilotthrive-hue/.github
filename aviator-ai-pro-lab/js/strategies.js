...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Consolidated single-pass metric accumulation and fast math rounding
    // to avoid redundant array allocations (slice, filter) and multiple reduce passes (~4x faster).
    let sum = 0;
    let recentSum = 0;
    let lowCount = 0;
    const recentStart = len > 5 ? len - 5 : 0;
    const recentLen = len - recentStart;
