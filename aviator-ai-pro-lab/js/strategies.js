...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Consolidate metrics (avg, variance, recentAvg, lowCrashRatio) into a single O(N) pass.
    ...
    const avg = sum / len;
    // Single-pass variance formula: (sumSq / count) - (avg * avg), with Math.max(0, ...) for numerical stability.
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    // BOLT OPTIMIZATION: Shallow copy originalParams before the loop to prevent random walk parameter mutation.
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        // BOLT OPTIMIZATION: Pass includeResults: false to avoid heavy array allocations during iterations.
    ...
    // BOLT OPTIMIZATION: Run a final high-fidelity backtest with best parameters and includeResults: true for UI rendering.
    if (bestParams) {
