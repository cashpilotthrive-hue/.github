...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Consolidate multiple passes (avg, variance, lowCrashRatio) into one O(N) loop
    let sum = 0;
    let lowCrashCount = 0;
    let recentSum = 0;
    ...
    // BOLT OPTIMIZATION: Replace toFixed() with mathematical rounding
    return {
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    // BOLT OPTIMIZATION: Captured base params to avoid mutation issues
    const originalParams = { ...strategy.params };

    for (let i = 0; i < iterations; i++) {
      // Use originalParams as base for randomization to avoid drift
      const params = this._randomizeParams(strategyKey, originalParams);
      this.strategies[strategyKey].params = params;

      try {
        // BOLT OPTIMIZATION: Run backtest without full results array for 80% faster iterations
    ...
    // BOLT OPTIMIZATION: Perform one final full backtest for the best params to include full results for UI display
    if (bestParams) {
