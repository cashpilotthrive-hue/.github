...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // into a single O(N) loop to eliminate redundant array allocations (.slice, .filter)
    // and multiple .reduce passes.
    let sum = 0;
    let sumSq = 0;
    let recentSum = 0;
    let lowCrashCount = 0;
    ...
    // BOLT OPTIMIZATION: Use direct Math.round instead of parseFloat(x.toFixed(n)) for faster numerical operations
    return {
  ...
  _randomizeParams(key, baseParams) {
    ...
    // BOLT OPTIMIZATION: Use direct Math.round instead of parseFloat(x.toFixed(n))
