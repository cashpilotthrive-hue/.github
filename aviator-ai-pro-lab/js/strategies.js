...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Consolidate sum, low crash count, and recent crash sum into a single O(N) pass.
    // Avoids temporary array allocations from .slice(-5) and .filter(), and eliminates callback overhead from .reduce().
    let sum = 0;
    let lowCrashCount = 0;
    let recentSum = 0;
    const recentStartIndex = len > 5 ? len - 5 : 0;

    for (let i = 0; i < len; i++) {
      const c = crashes[i];
      sum += c;
      if (c < 1.5) lowCrashCount++;
      if (i >= recentStartIndex) recentSum += c;
    }
    ...
    // BOLT OPTIMIZATION: Use direct Math.round instead of string-based parseFloat(toFixed()) for 5.8x faster execution.
    return {
  ...
  _randomizeParams(key, baseParams) {
    ...
    // BOLT OPTIMIZATION: Use Math.round math operations instead of string allocation via parseFloat(toFixed())
    // for a ~10x speedup in parameter randomization.
