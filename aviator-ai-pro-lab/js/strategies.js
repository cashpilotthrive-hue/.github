...
class StrategyEngine {
  ...
  backtest(strategyKey, crashPoints, bankroll = 1000, includeResults = true) {
    ...
    for (let i = 0; i < crashPoints.length; i++) {
      ...
      // BOLT OPTIMIZATION: Only build results array when requested (e.g., skip during optimization)
      if (includeResults) {
        results.push({
          round: i + 1,
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        // BOLT OPTIMIZATION: Skip results array generation for internal optimization passes
    ...
    // Restore original and perform one final backtest with full results for the best set
    ...
    if (bestParams) {
      ...
      this.strategies[strategyKey].params = strategy.params;
    }
