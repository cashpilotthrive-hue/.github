...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Consolidate sum, lowCrashCount, and recentSum into a single loop
    // to avoid redundant array allocations (slice/filter) and multiple reduce calls (~4x speedup).
    let sum = 0;
    let lowCrashCount = 0;
    let recentSum = 0;
    ...
    // BOLT OPTIMIZATION: Use Math.round instead of toFixed for fast numerical rounding
    return {
