...
class StrategyEngine {
  ...
  backtest(strategyKey, crashPoints, bankroll = 1000, options = {}) {
    ...
    for (let i = 0; i < crashPoints.length; i++) {
      ...
      // BOLT OPTIMIZATION: Skip results array allocation during AI optimization to dramatically reduce memory/CPU overhead.
      if (includeResults) {
        results.push({
          round: i + 1,
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Consolidate average, variance, recent five average, and low crash ratio calculation in a single O(N) pass.
    let sum = 0;
    let sumSq = 0;
    let sumRecent = 0;
    let lowCrashCount = 0;
    ...
    const avg = sum / len;
    // Using single-pass variance formula: (sum(x^2) / n) - (avg^2) with numerical stability safeguard
    const variance = Math.max(0, (sumSq / len) - (avg * avg));
    const volatility = Math.sqrt(variance);

    const recentAvg = sumRecent / recentLimit;
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    // Preserve original params to prevent optimization from permanently altering current settings.
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        // BOLT OPTIMIZATION: Run fast backtest without allocating results array to speed up optimization runs.
    ...
    // Restore original strategy params
    this.strategies[strategyKey] = { ...strategy, params: originalParams };

    // Run a final high-fidelity backtest with full results enabled so that UI visualization works correctly.
    if (bestParams) {
      const tempStrategy = { ...this.strategies[strategyKey], params: bestParams };
      this.strategies[strategyKey] = tempStrategy;
      const fullBestResult = this.backtest(strategyKey, crashPoints, bankroll, { includeResults: true });
      fullBestResult.score = bestScore;
      bestResult = fullBestResult;
      // Restore original strategy params again after final run
      this.strategies[strategyKey] = { ...strategy, params: originalParams };
    }
