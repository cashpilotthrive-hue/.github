...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // to calculate mean, variance, momentum, and crash ratios simultaneously.
    let sum = 0;
    let sumSq = 0;
    let lowCrashCount = 0;
    let recentSum = 0;
    const recentThreshold = len - 5;
    ...
    for (let i = 0; i < len; i++) {
      ...
      if (i >= recentThreshold) recentSum += c;
    }
    ...
    // BOLT OPTIMIZATION: Replace slow toFixed() calls with faster mathematical rounding
    return {
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    // BOLT OPTIMIZATION: Save original params to restore them later and use as a stable base for randomization
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        // BOLT OPTIMIZATION: Run backtest without generating results array to save memory/CPU
    ...
    // BOLT OPTIMIZATION: Final backtest with best params to include full result history for UI
    let finalBestResult = bestResult;
    if (bestParams) {
      const score = bestResult.score;
      strategy.params = bestParams;
      finalBestResult = { ...this.backtest(strategyKey, crashPoints, bankroll, true), score };
      strategy.params = originalParams;
    }
