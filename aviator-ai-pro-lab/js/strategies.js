...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Consolidated average, variance, recentAvg, and lowCrashRatio
    // into a single O(N) pass to eliminate redundant array iterations and allocations (.slice, .filter).
    let sum = 0;
    let sumSq = 0;
    let recentSum = 0;
    let lowCrashCount = 0;
    const recentStartIndex = len - 5;
