...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // calculations into a single O(N) pass, avoiding multiple array allocations (slice/filter)
    // and reduce iterations to optimize AI backtesting and iteration throughput.
    const len = crashes.length;
    let sum = 0;
    let sumSq = 0;
    let lowCrashCount = 0;
    let recentSum = 0;
    const recentLimit = len < 5 ? len : 5;
