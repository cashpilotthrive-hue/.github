...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    const avg = sum / count;
    const variance = Math.max(0, (sumSq / count) - (avg * avg)); // Numerically stable variance
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    // Run a final high-fidelity backtest with best parameters to get full results
    if (bestParams) {
      const score = bestResult ? bestResult.score : null;
      ...
      if (score !== null) bestResult.score = score;
    }
