...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Consolidate sum, recent sum, and low crash counting into a single O(N) loop.
    // Avoids redundant array allocations (slice, filter) and multiple reduce passes.
    let sum = 0;
    let lowCount = 0;
    let recentSum = 0;
    const recentCount = len > 5 ? 5 : len;
    ...
    // BOLT OPTIMIZATION: Fast numerical rounding using Math.round instead of parseFloat(toFixed()).
    return {
