...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    return {
      ...
      // BOLT OPTIMIZATION: Use Math.round instead of slow toFixed() calls
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        // BOLT OPTIMIZATION: Skip result array creation during optimization iterations
    ...
    // Perform a final backtest with the best parameters to get the full results for the UI
    if (bestParams) {
