...
class StrategyEngine {
  ...
  backtest(strategyKey, crashPoints, bankroll = 1000, includeResults = true) {
    ...
    for (let i = 0; i < crashPoints.length; i++) {
      ...
      // BOLT OPTIMIZATION: Optionally skip result collection to eliminate millions of object allocations during optimization.
      if (includeResults) {
        results.push({
          round: i + 1,
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Consolidate multiple array passes (avg, variance, recentAvg, lowCrashRatio)
    // into a single O(N) pass to significantly reduce overhead in the simulation loop.
    let sum = 0;
    let sumSq = 0;
    let recentSum = 0;
    let lowCrashCount = 0;
    ...
    const avg = sum / count;
    // Population variance formula: (sumSq / n) - (avg^2)
    ...
    // BOLT OPTIMIZATION: Use mathematical rounding instead of slow toFixed() calls.
    return {
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        // BOLT OPTIMIZATION: Disable result collection during optimization iterations to speed up the process.
