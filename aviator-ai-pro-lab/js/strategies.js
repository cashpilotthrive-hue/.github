...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // Consolidate statistics calculations into a single O(N) pass for high performance
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    // Capture original parameters via shallow copy to prevent degrading into a dependent random walk
    ...
    for (let i = 0; i < iterations; i++) {
      // Randomize parameters starting from a consistent original base
      const params = this._randomizeParams(strategyKey, originalParams);
      this.strategies[strategyKey].params = params;

      try {
        // Use high-performance optimized simulation without result array creation
    ...
    // Run a final high-fidelity backtest for the identified optimal parameters to fully populate bestResult
    let bestResult = null;
    if (bestParams) {
      ...
      // Ensure we restore original parameters after high-fidelity pass
      this.strategies[strategyKey].params = originalParams;
    }
