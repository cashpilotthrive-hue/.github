...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Calculate avg, volatility, recentAvg, and lowCrashRatio in a single O(N) pass
    // to avoid redundant array allocations (slice, filter) and multiple reduce calls.
    let sum = 0;
    let sumSq = 0;
    let lowCount = 0;
    let sumRecent5 = 0;
    const recent5Start = len > 5 ? len - 5 : 0;
    const recent5Count = len - recent5Start;
    ...
    for (let i = 0; i < len; i++) {
      ...
      sumSq += c * c;
      if (c < 1.5) lowCount++;
      if (i >= recent5Start) sumRecent5 += c;
    }
    ...
    const recentAvg = sumRecent5 / recent5Count;
    ...
    // BOLT OPTIMIZATION: Fast math rounding instead of parseFloat(toFixed()) string formatting
    return {
