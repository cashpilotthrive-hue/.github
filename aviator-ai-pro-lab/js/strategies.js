...
class StrategyEngine {
  ...
  /**
   * Helper function to round values mathematically instead of toFixed for high performance
   */
  ...
  backtest(strategyKey, crashPoints, bankroll = 1000, options = {}) {
    ...
    for (let i = 0; i < crashPoints.length; i++) {
      ...
      // BOLT OPTIMIZATION: Unused results parameter removed from _updateState and call site
  ...
  /**
   * AI-based crash pattern analysis optimized for single O(N) pass
   */
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Consolidate stats calculation into a single O(N) loop
    let sum = 0;
    let sumSq = 0;
    let recentSum = 0;
    let lowCrashCount = 0;
    ...
    const avg = sum / count;
    // Single-pass variance calculation with numerical stability guard
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    // BOLT OPTIMIZATION: Shallow copy original params before loop to prevent drift
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        // BOLT OPTIMIZATION: Run backtest with includeResults: false to save memory allocation
    ...
    // Restore original params before high-fidelity backtest
    this.strategies[strategyKey] = { ...strategy, params: originalParams };

    if (bestParams) {
      // Temporarily set to best params to run high-fidelity backtest
