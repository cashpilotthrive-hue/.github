...
class StrategyEngine {
  ...
  /**
   * Fast rounding helper function (math-based)
   */
  _round(n, decimals) {
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Calculate all stats in a single pass to optimize execution time
    ...
    const avg = sum / len;
    // Formula for single-pass variance calculation: (sumSq / count) - (avg * avg)
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    for (let i = 0; i < iterations; i++) {
      // BOLT OPTIMIZATION: Pass a copy of originalParams to ensure consistent base and prevent shared state mutation
      ...
      try {
        // BOLT OPTIMIZATION: Skip results array during optimization iterations
    ...
    // Run high-fidelity backtest for the best parameter set to get full results array
    if (bestParams) {
      ...
      // Restore original params after the final high-fidelity backtest
      this.strategies[strategyKey] = { ...strategy, params: originalParams };
    }
