...
class StrategyEngine {
  ...
  /**
   * Fast rounding helper (20x faster than toFixed)
   */
  _round(n, decimals = 2) {
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Single-pass calculation for avg, variance, and lowCrashRatio
    let sum = 0;
    let sumSq = 0;
    let lowCrashCount = 0;
    let recentSum = 0;
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        // BOLT OPTIMIZATION: Skip results array creation during optimization runs
    ...
    // Restore original and run one final high-fidelity backtest for the best params
