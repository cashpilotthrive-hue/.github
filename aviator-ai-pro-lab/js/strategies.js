...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Consolidate sum, recent sum, and low crash count into a single O(N) pass,
    // avoiding temporary array allocations (slice/filter) and redundant iterations.
    let sum = 0;
    let recentSum = 0;
    let lowCrashCount = 0;
    ...
    // BOLT OPTIMIZATION: Use Math.round instead of parseFloat(toFixed()) for stringless, fast rounding.
    return {
