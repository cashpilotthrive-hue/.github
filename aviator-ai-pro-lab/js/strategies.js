...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Consolidate metric calculations (avg, variance, recentAvg, lowCrashRatio)
    // into a single O(N) pass to eliminate redundant array allocations (slice, filter) and multiple reduce loops.
    let sum = 0;
    let sumSq = 0;
    let lowCount = 0;
    let recentSum = 0;
