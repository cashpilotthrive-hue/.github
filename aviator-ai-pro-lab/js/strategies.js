...
class StrategyEngine {
  ...
  backtest(strategyKey, crashPoints, bankroll = 1000, options = {}) {
    ...
    for (let i = 0; i < crashPoints.length; i++) {
      ...
      // BOLT OPTIMIZATION: Skip results array population when includeResults is false to save millions of allocations.
      if (includeResults) {
        results.push({
          round: i + 1,
  ...
  _aiAnalyze(state) {
    ...
    // in a single O(N) pass to avoid redundant .reduce, .slice, and .filter allocations.
    ...
    const recentLen = Math.min(len, 5);
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    // Capture original params to avoid randomization side effects
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        // BOLT OPTIMIZATION: Run fast backtest (includeResults: false) to skip millions of item allocations
    ...
    // Generate a single high-fidelity run with the best parameters to populate the full results
