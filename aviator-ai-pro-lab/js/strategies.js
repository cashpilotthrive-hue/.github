...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Calculate average, variance, recent average and low crash ratio in a single pass O(N) loop
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    // Run a final high-fidelity backtest with includeResults: true for the chosen optimal params
    if (bestParams && bestResult) {
      ...
      fullResult.score = bestResult.score; // restore the previously calculated score
