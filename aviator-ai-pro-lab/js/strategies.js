...
class StrategyEngine {
  ...
  backtest(strategyKey, crashPoints, bankroll = 1000, options = {}) {
    ...
    for (let i = 0; i < crashPoints.length; i++) {
      ...
      // BOLT OPTIMIZATION: Skip expensive result object creation during iterative optimization
      if (includeResults) {
        results.push({
          round: i + 1,
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Calculate all metrics in a single O(N) pass to avoid redundant array iterations
    let sum = 0;
    let sumSq = 0;
    let recentSum = 0;
    let lowCrashCount = 0;
    const recentCutoff = len - 5;

    for (let i = 0; i < len; i++) {
      const c = crashes[i];
      sum += c;
      sumSq += c * c;
      if (i >= recentCutoff) recentSum += c;
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        // BOLT OPTIMIZATION: Use includeResults: false during optimization to speed up simulation
    ...
    // Restore original params, then run one final backtest with best params to get full results for UI
