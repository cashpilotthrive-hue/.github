...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Calculate avg, sumSq (variance), recentAvg, and lowCrashRatio
    // in a single O(N) loop to eliminate redundant array allocations (slice/filter)
    // and multiple reduce iterations during AI strategy execution.
    let sum = 0;
    let sumSq = 0;
    let recentSum = 0;
    ...
    // BOLT OPTIMIZATION: Use direct math rounding instead of parseFloat(toFixed()) string conversions
    return {
