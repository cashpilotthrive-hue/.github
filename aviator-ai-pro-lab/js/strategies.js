...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // into a single O(N) pass to avoid redundant array allocations (slice, filter) and multiple reduces.
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    // BOLT OPTIMIZATION: Capture original parameters to avoid state bleed and cumulative randomized drift
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        // BOLT OPTIMIZATION: Disable results array population during iterations to avoid memory pressure and garbage collection
    ...
    // Always restore the original params to keep StrategyEngine state clean and consistent
    strategy.params = originalParams;

    // Run a final high-fidelity backtest with the best parameters so the UI has full result details to render
    if (bestParams) {
