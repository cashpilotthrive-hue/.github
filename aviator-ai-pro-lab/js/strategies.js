...
class StrategyEngine {
  ...
  backtest(strategyKey, crashPoints, bankroll = 1000, includeResults = true) {
    ...
    for (; i < crashPoints.length; i++) {
      ...
      // BOLT OPTIMIZATION: Skip results collection during optimization to save memory/CPU.
      if (includeResults) {
        results.push({
          round: i + 1,
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Consolidate multiple array traversals (reduce, filter, slice)
    // into a single O(N) pass to calculate avg, variance, recentAvg, and lowCrashRatio.
    let sum = 0;
    let sqSum = 0;
    let recentSum = 0;
    let lowCrashCount = 0;
    ...
    const avg = sum / len;
    const variance = (sqSum / len) - (avg * avg);
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        // BOLT OPTIMIZATION: Call backtest with includeResults = false for faster iterations.
