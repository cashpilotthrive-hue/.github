...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // to minimize array iterations and object allocations.
    let sum = 0;
    let sumSq = 0;
    let lowCrashCount = 0;
    let recentSum = 0;
    ...
    for (let i = 0; i < len; i++) {
      ...
      if (i >= len - recentLen) recentSum += c;
    }
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        // BOLT OPTIMIZATION: Pass false as the fourth parameter to skip building the results array
        // which is not needed during optimization runs, significantly reducing allocations.
