...
class StrategyEngine {
  ...
  backtest(strategyKey, crashPoints, bankroll = 1000, includeResults = true) {
    ...
    for (let i = 0; i < crashPoints.length; i++) {
      ...
      // BOLT OPTIMIZATION: Skip results array creation when not needed to reduce allocations
      if (includeResults) {
        results.push({
          round: i + 1,
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Consolidate multiple passes (avg, variance, momentum, ratio) into a single O(N) pass.
    let sum = 0;
    let sumSq = 0;
    let recentSum = 0;
    let lowCrashCount = 0;
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        // BOLT OPTIMIZATION: Use includeResults: false for internal iterations to save memory
    ...
    // BOLT OPTIMIZATION: Perform one final high-fidelity backtest with the best parameters found
    let finalResult = null;
    if (bestParams) {
      this.strategies[strategyKey].params = bestParams;
      finalResult = this.backtest(strategyKey, crashPoints, bankroll, true);
