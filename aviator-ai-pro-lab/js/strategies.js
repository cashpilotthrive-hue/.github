...
class StrategyEngine {
   ...
   */
  backtest(strategyKey, crashPoints, bankroll = 1000, options = { includeResults: true }) {
    ...
    for (let i = 0; i < crashPoints.length; i++) {
      ...
      // BOLT OPTIMIZATION: Conditionally push to results to save memory during iterations
      if (includeResults) {
        results.push({
          round: i + 1,
    ...
    return {
      ...
      peakBankroll: this._round(peakBankroll).toFixed(2)
    };
  }
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Consolidate multiple array passes into a single O(N) pass
    let sum = 0;
    let sumSq = 0;
    let lowCrashCount = 0;
    let recentSum = 0;
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (i >= count - recentCount) recentSum += c;
    }

    const avg = sum / count;
    const variance = (sumSq / count) - (avg * avg);
    const volatility = Math.sqrt(Math.max(0, variance));
    const momentum = (recentSum / recentCount) - avg;
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    // BOLT OPTIMIZATION: Capture original params to restore later
    const originalParams = { ...strategy.params };
    let bestResult = null;
    let bestParams = null;

    for (let i = 0; i < iterations; i++) {
      // BOLT OPTIMIZATION: Always use base original params for randomization
      const params = this._randomizeParams(strategyKey, originalParams);
      this.strategies[strategyKey].params = params;

      try {
        // BOLT OPTIMIZATION: Skip result array creation during iterations
    ...
    // BOLT OPTIMIZATION: Run a final full backtest for the best params to get results for UI
    if (bestParams) {
      const finalParams = { ...bestParams };
      this.strategies[strategyKey].params = finalParams;
  ...
  _randomizeParams(key, baseParams) {
    ...
    params.baseBet = Math.floor(rand(1, 50));

    switch (key) {
      case 'martingale':
        params.multiplier = this._round(rand(1.5, 3.0), 1);
        params.maxBet = Math.floor(rand(200, 2000));
        break;
      case 'antiMartingale':
        params.multiplier = this._round(rand(1.5, 3.0), 1);
        ...
        break;
      case 'dalembert':
        params.unitSize = Math.floor(rand(1, 20));
        break;
      case 'kelly':
