...
class StrategyEngine {
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Consolidate multiple array passes into a single O(N) loop
    let sum = 0;
    let sumSq = 0;
    let lowCrashCount = 0;
    let recentSum = 0;
    const recentWindow = Math.min(numCrashes, 5);
    ...
    for (let i = 0; i < numCrashes; i++) {
      ...
      if (i >= numCrashes - recentWindow) recentSum += c;
    }

    const avg = sum / numCrashes;
    // Calculate variance using sum of squares: E[X^2] - (E[X])^2
    const variance = (sumSq / numCrashes) - (avg * avg);
    const volatility = Math.sqrt(Math.max(0, variance));
    const recentAvg = recentSum / recentWindow;
    const momentum = recentAvg - avg;
    const lowCrashRatio = lowCrashCount / numCrashes;
    ...
    const confidence = Math.min(0.95, 0.3 + (numCrashes / state.adaptiveWindow) * 0.5 - volatility * 0.05);
    ...
    return {
      ...
      // BOLT OPTIMIZATION: Faster mathematical rounding
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    const initialParams = { ...strategy.params };

    let bestResult = null;
    let bestParams = null;

    for (let i = 0; i < iterations; i++) {
      const params = this._randomizeParams(strategyKey, initialParams);
      // BOLT OPTIMIZATION: Assign parameters directly to temporary strategy for faster access
      this.strategies[strategyKey].params = params;

      try {
        // BOLT OPTIMIZATION: Skip generating results array during optimization iterations
    ...
    this.strategies[strategyKey].params = initialParams;

    // BOLT OPTIMIZATION: Run a final full backtest for the best result to provide detailed data for UI
    if (bestParams) {
      ...
      bestResult = { ...finalResult, score: bestScore };
      this.strategies[strategyKey].params = initialParams;
    }
