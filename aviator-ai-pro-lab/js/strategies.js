...
class StrategyEngine {
  ...
  backtest(strategyKey, crashPoints, bankroll = 1000, options = {}) {
    ...
    for (let i = 0; i < crashPoints.length; i++) {
      ...
      // BOLT OPTIMIZATION: Skip results array creation during optimization for 3x speedup
      if (includeResults) {
        results.push({
          round: i + 1,
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        ...
        // to skip results array creation and improve performance by ~3x.
    ...
    if (bestParams) {
      // Run one final full backtest with best parameters for the UI
      ...
      // Restore original params to the strategy object
