...
class StrategyEngine {
  ...
  /**
   * Helper to round numbers efficiently
   */
  _round(n, decimals = 2) {
  ...
  backtest(strategyKey, crashPoints, bankroll = 1000, options = {}) {
    ...
    for (let i = 0; i < crashPoints.length; i++) {
      ...
      // BOLT OPTIMIZATION: Skip results array population and rounding when results are not needed
      if (includeResults) {
        results.push({
          round: i + 1,
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Single pass O(N) calculation for all metrics
    let sum = 0;
    let sumSq = 0;
    let recentSum = 0;
    let lowCrashes = 0;
    const recentThreshold = count - 5;
    ...
    const recentAvg = recentSum / Math.min(count, 5);
    const momentum = recentAvg - avg;
    const lowCrashRatio = lowCrashes / count;
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    let bestResult = null;
    let bestParams = null;
    // Capture original params for restoration and consistent randomization
    const originalParams = { ...strategy.params };

    for (let i = 0; i < iterations; i++) {
      const params = this._randomizeParams(strategyKey, originalParams);
      // Update the strategy in the engine for backtest to use
      this.strategies[strategyKey].params = params;

      try {
        // BOLT OPTIMIZATION: Use includeResults: false for faster iterative runs
    ...
    // Perform one final full backtest for the best parameters to get full results for UI
    if (bestParams) {
