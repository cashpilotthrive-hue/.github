...
class StrategyEngine {
  ...
  backtest(strategyKey, crashPoints, bankroll = 1000, options = {}) {
    ...
    for (let i = 0; i < crashPoints.length; i++) {
      ...
      if (includeResults) {
        // BOLT OPTIMIZATION: Use fast math-based _round helper instead of toFixed
        results.push({
          round: i + 1,
  ...
  _aiAnalyze(state) {
    ...
    // BOLT OPTIMIZATION: Calculate average, variance, recent average, and low crash ratio in a single O(N) pass.
    let sum = 0;
    let sumSq = 0;
    let recentSum = 0;
    let lowCrashCount = 0;
    ...
    const avg = sum / len;
    // variance = E[X^2] - (E[X])^2 = (sumSq / len) - (avg * avg)
  ...
  optimize(strategyKey, crashPoints, bankroll = 1000, iterations = 50) {
    ...
    // Capture original parameters via shallow copy to ensure consistency and prevent mutations.
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        // BOLT OPTIMIZATION: Run fast backtest without results array generation during optimization iterations.
    ...
    // Restore original parameter state in the engine.
    strategy.params = originalParams;

    // Run a final high-fidelity backtest with results included to return fully populated data for the UI.
    if (bestParams) {
