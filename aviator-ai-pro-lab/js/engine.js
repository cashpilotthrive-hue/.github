...
class AviatorEngine {
  ...
  _generateSeed() {
    // BOLT OPTIMIZATION: Use a simple loop for hex conversion to avoid Array.from/join overhead
    const arr = new Uint32Array(4);
    crypto.getRandomValues(arr);
    let hex = '';
    for (let i = 0; i < arr.length; i++) {
      hex += (arr[i] >>> 0).toString(16).padStart(8, '0');
    }
    return hex;
  }
  ...
  simulateRound(betAmount, cashOutAt) {
    ...
    // BOLT OPTIMIZATION: Use a mathematical rounding helper instead of slow toFixed(2)
    const round = {
   ...
   */
  getStats() {
    // BOLT OPTIMIZATION: Consolidate ~20 passes into a single O(N) loop
    ...
    // For Sharpe Ratio (Single pass variance calculation)
    let sumProfitSq = 0;
    ...
    if (len >= 2) {
      ...
      const sampleVariance = (sumProfitSq - (sumProfit * sumProfit / len)) / (len - 1);
    ...
    return {
      ...
      profitFactor: isFinite(profitFactor) ? profitFactor.toFixed(2) : profitFactor.toString()
    };
  }
