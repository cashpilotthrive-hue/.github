...
// BOLT OPTIMIZATION: Hoist top-level constants and helper to avoid re-declarations
const _hex8 = (v) => (v >>> 0).toString(16).padStart(8, '0');
const E_2_52 = 4503599627370496; // 2 ** 52
...
class AviatorEngine {
  ...
  // BOLT OPTIMIZATION: Reuse instance Uint32Array buffer to avoid allocation churn
  _generateSeed() {
  ...
  getStats() {
    ...
    // BOLT OPTIMIZATION: Use Float64Array for memory locality and fast native sorting
  ...
  _median(arr) {
    // BOLT OPTIMIZATION: Fast native sort for Float64Array without JS callback overhead
    const sorted = arr instanceof Float64Array ? new Float64Array(arr) : [...arr].sort((a, b) => a - b);
    if (sorted instanceof Float64Array) sorted.sort();
