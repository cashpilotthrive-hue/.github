...
class AviatorEngine {
  ...
  getStats() {
    ...
    // BOLT OPTIMIZATION: Consolidate metrics into a single O(N) pass and use performant rounding
    // to avoid multiple array iterations and expensive toFixed calls.
    let winsCount = 0;
    let totalProfit = 0;
    let totalCrash = 0;
    ...
    for (let i = 0; i < totalRounds; i++) {
      ...
      maxDD = Math.max(maxDD, peak - totalProfit);
    ...
    const variance = totalRounds > 1 ? varianceSum / (totalRounds - 1) : 0;
    const std = Math.sqrt(variance);
    const sharpe = std === 0 ? 0 : (meanProfit / std) * Math.sqrt(252);
