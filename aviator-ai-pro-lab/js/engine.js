...
class AviatorEngine {
  ...
  getStats() {
    ...
    // BOLT OPTIMIZATION: Calculate most metrics in a single O(N) pass to avoid redundant iterations
    ...
    const meanProfit = totalProfit / totalRounds;

    return {
      totalRounds,
      winRate: this._round(winsCount / totalRounds * 100, 1),
      totalProfit: this._round(totalProfit, 2),
      avgCrash: this._round(totalCrash / totalRounds, 2),
      maxCrash: this._round(maxCrash, 2),
      minCrash: this._round(minCrash, 2),
      medianCrash: this._round(this._median(crashes), 2),
      longestWinStreak,
      longestLoseStreak,
      avgProfit: this._round(meanProfit, 2),
      maxDrawdown: this._round(maxDrawdown, 2),
      sharpeRatio: this._round(this._sharpeRatio(profits, meanProfit), 3),
      profitFactor: grossLosses === 0 ? (grossWins > 0 ? 'Infinity' : '0.00') : this._round(grossWins / grossLosses, 2)
    };
  }
  ...
  _sharpeRatio(profits, mean) {
    ...
    for (let i = 0; i < profits.length; i++) {
      sumSqDiff += Math.pow(profits[i] - mean, 2);
    }
    const variance = sumSqDiff / (profits.length - 1);
