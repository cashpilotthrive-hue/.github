...
class AviatorEngine {
  ...
  _generateSeed() {
    // BOLT OPTIMIZATION: Reuse instance Uint32Array buffer to avoid allocation overhead during seed generation
    if (!this._seedBuffer) {
  ...
  generateCrashPoint() {
    ...
    // BOLT OPTIMIZATION: Compute 52-bit integer hash directly without 32-char hex string allocations or parseInt parsing
    const h = this._hash52(hashInput);
    const result = (100 * E_2_52 - h) / (E_2_52 - h);
    return Math.max(1.0, Math.floor(result) / 100);
  }
  ...
  getStats() {
    ...
    // and use Float64Array for memory-efficient crash point accumulation.
  ...
  _median(arr) {
    // BOLT OPTIMIZATION: Use Float64Array native sort for median calculation
