...
class AviatorEngine {
  ...
  simulateRound(betAmount, cashOutAt) {
    ...
    // and avoid repeated parseFloat calls.
    const round = {
  ...
  getStats() {
    ...
    // BOLT OPTIMIZATION: Consolidate all metrics into a single O(N) pass to avoid
    // redundant array iterations (map/filter/reduce) and memory allocations.
    // This also avoids Math.max(...spread) which can stack overflow on large history.
    let totalWins = 0;
    let totalProfit = 0;
    let totalCrash = 0;
    ...
    // Sharpe Ratio requires standard deviation (second pass for precision)
    let varianceSum = 0;
    for (let i = 0; i < len; i++) {
      varianceSum += Math.pow(this.history[i].profit - avgProfit, 2);
    }
    const std = len < 2 ? 0 : Math.sqrt(varianceSum / (len - 1));
    const sharpe = std === 0 ? 0 : (avgProfit / std) * Math.sqrt(252);

    return {
      totalRounds: len,
      winRate: (totalWins / len * 100).toFixed(1),
      ...
      profitFactor: (grossLosses === 0 ? (grossWins > 0 ? Infinity : 0) : grossWins / grossLosses).toFixed(2)
    };
  }
