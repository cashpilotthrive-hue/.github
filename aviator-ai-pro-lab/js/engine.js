...
// BOLT OPTIMIZATION: Hoist constant exponential power and helper function
// to avoid recreating functions and recalculating constants in hot paths.
...
class AviatorEngine {
  ...
  // BOLT OPTIMIZATION: Reuse instance Uint32Array buffer with lazy initialization
  // to avoid allocating typed arrays and JS objects on every seed generation.
  _generateSeed() {
    if (!this._seedBuffer) this._seedBuffer = new Uint32Array(4);
    crypto.getRandomValues(this._seedBuffer);
    return (
      _hex8(this._seedBuffer[0]) +
      _hex8(this._seedBuffer[1]) +
      _hex8(this._seedBuffer[2]) +
      _hex8(this._seedBuffer[3])
    );
  }
  ...
  generateCrashHistory(count) {
    // BOLT OPTIMIZATION: Pre-allocate array capacity to prevent dynamic resizing overhead.
  ...
  getStats() {
    ...
    // and use a Float64Array for memory-dense crash storage to avoid array allocations.
  ...
  // BOLT OPTIMIZATION: Use Float64Array native sorting to eliminate JS comparison function overhead.
  _median(arr) {
