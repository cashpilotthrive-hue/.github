...
// BOLT OPTIMIZATION: Hoist 2^52 constant for crash calculation
...
class AviatorEngine {
  ...
  _generateSeed() {
    // BOLT OPTIMIZATION: Reuse instance buffer lazily to avoid Uint32Array allocations
    if (!this._seedBuffer) {
  ...
  generateCrashPoint() {
    ...
    // BOLT OPTIMIZATION: Compute 52-bit hash value directly without string allocation/slicing
