...
def _moderate_content(lowered_content: str) -> ModerationResponse:
    """Internal helper to perform moderation on pre-lowered content."""
...
def chat(payload: ChatRequest) -> ChatResponse:
    ...
    # BOLT OPTIMIZATION: Lower the user message once and reuse it for moderation
    # and tool checks to avoid redundant processing of potentially large strings.
    latest_user_message_lowered = latest_user_message.lower()

    moderation = _moderate_content(latest_user_message_lowered)
    ...
    tool_calls: list[dict[str, Any]] = []
    if payload.tools_enabled and "time" in latest_user_message_lowered:
        tool_calls.append(
            {
