...
def chat(payload: ChatRequest) -> ChatResponse:
    ...
    lowered_content = latest_user_message.lower()

    # BOLT OPTIMIZATION: Reuse the pre-lowered user message for both moderation and tool checks.
    # This avoids redundant .lower() calls on potentially large message contents.
    moderation = _moderate_content(latest_user_message, lowered_content=lowered_content)
    ...
    tool_calls: list[dict[str, Any]] = []
    if payload.tools_enabled and "time" in lowered_content:
        tool_calls.append(
            {
