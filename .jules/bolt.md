...
## 2026-03-26 - Efficient file idempotency in shell scripts
**Learning:** Using `cmp -s` is a fast way to verify if two files are identical before performing expensive operations like backups or network copies.
**Action:** Always check for file identity with `cmp -s` before overwriting or backing up configuration files in setup scripts.
