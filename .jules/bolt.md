...
## 2026-07-10 - Massive speedup by skipping result allocations in iterations
**Learning:** In high-iteration strategy optimization loops, the allocation and subsequent garbage collection of large result arrays (e.g., 1000 objects per iteration) is a primary bottleneck.
**Action:** Implement an 'includeResults: false' flag in simulation/backtest methods to skip data collection during search/optimization phases, only performing a full backtest once the optimal parameters are found.
