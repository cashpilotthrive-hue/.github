...
## 2026-05-22 - StrategyEngine Simulation Throughput
**Learning:** Consolidating multiple array passes (`reduce`, `filter`, `slice`) into a single `for` loop and skipping large object array allocations (`results`) during AI optimization iterations reduces latency by ~60-80%.
**Action:** Use a `includeResults` flag in backtest/simulation methods to skip logging/rounding when only final metrics are needed for scoring.
