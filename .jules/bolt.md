...
## 2026-02-12 - [SQLite Thread-Local Connection Pooling]
**Learning:** Repeatedly opening and closing SQLite connections, and re-executing PRAGMAs on every request, introduces significant overhead (~0.5ms to 4ms per request depending on environment).
**Action:** Implement thread-local connection caching using `threading.local()`. This reduces connection overhead by ~28x for writes and ~40x for reads, while remaining safe for FastAPI's multi-threaded worker model. Ensure `rollback()` is called on exception to keep cached connections clean.
