...
## 2026-04-21 - High-iteration simulation optimization
**Learning:** In simulation engines with high-iteration optimization loops (e.g., 80+ iterations of 1000 rounds), the primary bottlenecks are object allocations (results array) and redundant array traversals in statistical analysis.
**Action:** Implement an `includeResults: false` flag to skip result object creation during discarded iterations, and consolidate multi-pass statistical metrics (avg, variance, etc.) into a single O(N) loop.
