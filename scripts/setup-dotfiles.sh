...
copy_dotfile() {
    ...
    if [ -f "$DOTFILES_DIR/$file" ]; then
        # BOLT OPTIMIZATION: Use cmp -s for idempotency check to avoid redundant backups and copies.
        # This reduces warm-run time by skipping operations when files are already identical.
