...
copy_dotfile() {
    ...
    if [ -f "$DOTFILES_DIR/$file" ]; then
        # BOLT OPTIMIZATION: Use cmp -s to skip redundant backups and copies if files are identical.
        # This reduces warm-run execution time significantly by avoiding filesystem writes.
